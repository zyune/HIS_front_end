      ...
      <el-col :span="6">
        <div>
          ...
          <el-row class="grid-content bg-purple-light">
            <el-col :span="12">
              <div style="text-align: left">待检验列表</div>
            </el-col>
            <el-col :span="12">
    ...
    export default {
        ...
        methods: {
            ...
            dengji(index) {
                ...
                this.$put("/inspect/putItemHandleState", this.checkItemData[index].id).then((res) => {
            ...
            getWaitYjPatient() {
                this.$fetch("/inspect/getWaitYjPatient").then((res) => {
                    this.waitPatient = res.data;
                })
            },
            getPatientInfo(row) {
                this.nowapplication = row;
                this.$fetch("/inspect/getPatientSexAndAge", {blh: row.medicalRecord}).then((res) => {
                    this.patientInfoData.patientSex = res.data.patientSex;
                    this.patientInfoData.patientAge = res.data.patientAge;
                });
                this.$fetch("/inspect/getPatientOtherInfo", {id: row.id}).then((res) => {
                ...
                this.$fetch("/inspect/getCheckItem", {id: row.id}).then((res) => {
            ...
            postCheckResult() {
                this.$post("/inspect/postCheckResult", this.addform).then(res => {
                    console.log(this.addform)
                    alert("添加成功")
                })
            ...
            getPatient() {
                ...
                if (this.input1 != "") {
                    console.log(this.input1)
                    this.$fetch("/inspect/getWaitFyPatientById", {medicalRecord: this.input1}).then((res) => {
                        this.waitPatient = res.data;
                        alert("搜索成功")
                    })
                } else if (this.hzxm != "") {
                    console.log(this.hzxm);
                    this.$fetch("/inspect/getWaitFyPatientByName", {patientName: this.hzxm}).then((res) => {
                        this.waitPatient = res.data;
                        alert("搜索成功")
                    })
                }

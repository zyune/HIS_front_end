      ...
      <el-col :span="20">
        ...
        >
          ...
          <el-table-column prop="createTime" label="创建日期" min-width="100">
             <template slot-scope="scope">{{scope.row.createTime | dateTimeForm}}</template></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getAllCheckFee() {
      this.$fetch("/sf/listAllDisposeFee").then(res => {
        const data = res.data;
        console.log(data);
    ...
    getAllCheckFeeByPatientName() {
      this.$fetch("/sf/listDisposeFeeByPatientName", {

            ...
            <el-col :span="6">
                <div>
                    ...
                    <el-row class="grid-content bg-purple-light">
                        <el-col :span="12">
                            <div style="text-align: left">待检查列表</div>
                        </el-col>
                        <el-col :span="12">
    ...
    export default {
        ...
        methods: {
            ...
            dengji(index) {
                ...
                this.$put("/yj/putItemHandleState", this.checkItemData[index].id).then((res) => {
            ...
            getWaitYjPatient() {
                this.$fetch("/yj/getWaitYjPatient").then((res) => {
                    this.waitPatient = res.data;
                })
            },
            getPatientInfo(row) {
                this.nowapplication = row;
                this.$fetch("/yj/getPatientSexAndAge", {blh: row.medicalRecord}).then((res) => {
                    this.patientInfoData.patientSex = res.data.patientSex;
                    this.patientInfoData.patientAge = res.data.patientAge;
                });
                this.$fetch("/yj/getPatientOtherInfo", {id: row.id}).then((res) => {
                ...
                this.$fetch("/yj/getCheckItem", {id: row.id}).then((res) => {
            ...
            postCheckResult() {
                this.$post("/yj/postCheckResult", this.addform).then(res => {
                    console.log(this.addform)
                    alert("添加成功")
                })
            ...
            getPatient() {
                ...
                if (this.input1 != "") {
                    console.log(this.input1)
                    this.$fetch("/yj/getWaitFyPatientById", {medicalRecord: this.input1}).then((res) => {
                        this.waitPatient = res.data;
                        alert("搜索成功")
                    })
                } else if (this.hzxm != "") {
                    console.log(this.hzxm);
                    this.$fetch("/yj/getWaitFyPatientByName", {patientName: this.hzxm}).then((res) => {
                        this.waitPatient = res.data;
                        alert("搜索成功")
                    })
                }

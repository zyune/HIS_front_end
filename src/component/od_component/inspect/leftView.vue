<template>
  <div style="margin-top: 10px;margin-bottom: 20px;padding-left: 20px;padding-right: 20px">
    <h1 style="text-align: center;padding-bottom: 6px;">检验申请列表单</h1>
    <el-row>
      <el-col :span="6"><div>检验申请列表单</div></el-col>
    ...
    <el-row>
      <el-col :span="24">
        <el-row>
          <el-col :span="6"><div>检验项目列表</div></el-col>
    ...
    <el-dialog title="检验项目结果情况" :visible.sync="dialogResultVisible" >
      <el-row>
    ...
    <el-dialog title="新增检验项目" :visible.sync="dialogTableVisible" >
      <el-row>
        ...
        <el-col :span="24">
          <el-form>
            <el-form-item label="修改申请的名称">
              <el-input v-model="alertApplictionInfo.checkApplicationName" autocomplete="true" style="width: 180px"></el-input>
            </el-form-item>
    ...
    <rightTemplate ref="rightTemplate"></rightTemplate>
...
<script>
  ...
  export default {
    name: 'jianyan1',
    components: {
      saveDialogTemplate,rightTemplate
    },
    data() {
      return {
    ...
    methods: {
      openStack(){
        this.$refs.rightTemplate.openTemplateVisible()
      },
      ...
      sureUseCheckTemplateAppliction() {
        if(this.$store.getters.getInspectionPatient.isSeenDocator !=2 ){
          ...
          this.dialoguseCheckTemplate.checkType = 2;//检验申请
      ...
      sureAppliction() {
        ...
        this.checkApplication.checkType = 2;
      ...
      searchItemByName() {
        this.loading = true;
        var _this = this;
        this.$axios.get("/od/getInspectFemdItemByName", {
          params: {
      ...
      loadingPatientCheckInfo() {
        ...
        if (this.$store.getters.getInspectionPatient.medicalRecord == null) {
          ...
          this.$axios.get("/od/getInspectApplicationByblh", {
            params: {
    ...
    created() {
      ...
      this.loadingPatientCheckInfo();
      console.log("你已经开立了 应该是不能进行操作的呢，为什么还会进行操作呢");
      event.$on("commitPatient", (res) => {
        // 进行下通知，让这个方法进行获得下新的数据
        _this.xiangmuList = null;
        this.loadingPatientCheckInfo();
      });
      //当用户进行检验申请的时候，会触发这个逻辑，使得逻辑能够新增加一条记录

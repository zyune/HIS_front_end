<template>
    <div style="margin-top: 10px;margin-bottom: 20px;padding-left: 20px;padding-right: 20px">
      <h1 style="text-align: center;padding-bottom: 6px;">处置申请列表单</h1>
    <el-row>
      <el-col :span="6"><div>处置申请列表单</div></el-col>
    ...
    <el-row>
      <el-col :span="24">
        <el-row>
          <el-col :span="6"><div>处置项目列表</div></el-col>
    ...
    <el-dialog title="处置项目结果情况" :visible.sync="dialogResultVisible" >
      <el-row>
    ...
    <el-dialog title="新增处置项目" :visible.sync="dialogTableVisible" >
      <el-row>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      sureUseCheckTemplateAppliction() {
        if(this.$store.getters.getInspectionPatient.isSeenDocator !=2 ){
          ...
          this.dialoguseCheckTemplate.checkType = 3;//处置申请
      ...
      addCheckFemdItem(row) {
        if (this.applicationList[this.radio].checkApplication.useState == 1) {
          ...
          //把checKitem中的id设置为处置申请表的id,然后在后台获得，然后再设置为null
      ...
      sureAppliction() {
        ...
        this.checkApplication.checkType = 3;
      ...
      loadingPatientCheckInfo() {
        ...
        if (this.$store.getters.getInspectionPatient.medicalRecord == null) {
          ...
          this.$axios.get("/od/getDisposeApplicationByblh", {
            params: {
    ...
    created() {
      ...
      //当用户进行处置申请的时候，会触发这个逻辑，使得逻辑能够新增加一条记录
